...
func TestIamAccessKeySupplier_Resources(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			mocks: func(repo *repository.MockIAMRepository) {
				repo.On("ListAllAccessKeys").Return([]*iam.AccessKeyMetadata{}, nil)
			},
			err: nil,
		},
		{
			...
			mocks: func(repo *repository.MockIAMRepository) {
				repo.On("ListAllAccessKeys").Return([]*iam.AccessKeyMetadata{
					{
		...
		},
		{
			...
			dirName: "iam_access_key_empty",
			mocks: func(repo *repository.MockIAMRepository) {
				repo.On("ListAllAccessKeys").Return(nil, awserr.NewRequestFailure(nil, 403, ""))
			},
